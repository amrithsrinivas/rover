...
impl RoverClient {
    ...
    /// Returns the persistent API key and server info.
    ...
    /// Get server info (name, version, OS, uptime).
    ...
    /// Get current server metrics (CPU, RAM, disk).
    ...
    /// Get detailed info about a single app.
    ...
    /// Deploy an app (server-sent streaming response).
